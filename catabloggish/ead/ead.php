...
/* Upload EAD file to uploads folder */
...
function ead_options() {
  ...
  // Check for PHP XSL extension
  if (!extension_loaded('xsl')) {
    echo '<div id="message" class="error"><p><strong>In order to use the EAD Importer, the PHP extension XSL must be enabled.  Go here for <a href="http://code.google.com/p/catabloggish">more information</a></strong></p></div>'; 
    die;
  ...
  if ( $_REQUEST['error'] ) {
    ...
    // If none, create post with transformation result  
    ...
    if ( $id ) {
      ...
      //If we don't get a post id, something went wrong
    } else {
...
/* Transform EAD/XML to HTML for a WordPress post */
